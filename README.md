...
Kestrel is one app, one database and one deployment, served from four
hostnames:

| Host | Surface | Routes |
|---|---|---|
| `kestrel.com` | marketing | `routes/marketing.php` |
| `app.kestrel.com` | the operator app | `routes/app.php` |
| `book.kestrel.com/{slug}` | public booking | `routes/book.php` |
| `admin.kestrel.com` | super admin | `routes/admin.php` |
...
**By default there is no DNS setup.** With `APP_DOMAIN` empty, every surface is
served from `APP_URL` on the path prefix it used before the split — `/`,
`/diary`, `/book/{slug}`, `/admin`. `php artisan serve` and `php artisan test`
work as they always have.

To run with real subdomains, add the four hostnames to `/etc/hosts` and set
`APP_DOMAIN` plus the four `APP_URL_*` values. Both modes are in DEPLOY.md and
