...
{
    config([
        'app.domain' => 'kestrel.test',
        'app.subdomain_routing' => true,
        'app.surfaces.marketing' => 'http://kestrel.test',
        'app.surfaces.app' => 'http://app.kestrel.test',
        'app.surfaces.book' => 'http://book.kestrel.test',
        'app.surfaces.admin' => 'http://admin.kestrel.test',
    ]);
...
it('serves each surface from its own host', function () {
    ...
    $this->get('http://kestrel.test/')->assertOk();
    $this->get('http://kestrel.test/pricing')->assertOk();
});

it('does not serve marketing from the app host', function () {
    withSubdomains();

    $this->get('http://app.kestrel.test/pricing')->assertNotFound();
});

it('does not serve the app from the marketing host', function () {
    withSubdomains();

    $this->get('http://kestrel.test/diary')->assertNotFound();
});
...
it('does not serve the console from the app host', function () {
    ...
    $this->get('http://app.kestrel.test/')->assertNotFound();
    $this->get('http://app.kestrel.test/messages')->assertNotFound();
    $this->get('http://app.kestrel.test/failures')->assertNotFound();
});

it('does not serve the console from the marketing or booking hosts', function () {
    withSubdomains();

    $this->get('http://book.kestrel.test/failures')->assertNotFound();
    $this->get('http://kestrel.test/failures')->assertNotFound();
});
...
it('does not serve the booking page from the app host', function () {
    ...
    $this->get("http://app.kestrel.test/{$tenant->slug}")->assertNotFound();
});
...
it('serves the booking page from the booking host with no path prefix', function () {
    ...
    $this->get("http://book.kestrel.test/{$tenant->slug}")->assertOk();
});
...
it('keeps the fixed booking prefixes reachable ahead of the slug wildcard', function () {
    ...
    $this->get('http://book.kestrel.test/b/not-a-real-token')->assertNotFound();

    $matched = app('router')->getRoutes()->match(
        Illuminate\Http\Request::create('http://book.kestrel.test/b/some-token', 'GET'),
    );
...
it('builds a booking URL on the booking host', function () {
    ...
    expect(book_url($tenant))->toBe('http://book.kestrel.test/willow-street')
        ->and(Surface::bookUrlFor($tenant))->toBe('http://book.kestrel.test/willow-street');
});

it('builds each surface URL on its own host', function () {
    withSubdomains();

    expect(marketing_url())->toBe('http://kestrel.test')
        ->and(app_url('diary'))->toBe('http://app.kestrel.test/diary')
        ->and(admin_url())->toBe('http://admin.kestrel.test')
        ->and(book_url(null, 'b/abc'))->toBe('http://book.kestrel.test/b/abc');
});
...
it('will not read an app session on the console host', function () {
    ...
    $this->get('http://app.kestrel.test/login');
    $appCookie = config('session.cookie');

    $this->get('http://admin.kestrel.test/login');

    expect(config('session.cookie'))->not->toBe($appCookie)
        ->and(config('session.domain'))->toBe('admin.kestrel.test');
});
...
it('shows the impersonation banner on every app screen while active', function () {
    ...
    $handoff = $this->actingAs($admin)
        ->post("http://admin.kestrel.test/tenants/{$tenant->id}/impersonate")
    ...
    foreach (['diary', 'bookings', 'customers'] as $screen) {
        $this->get("http://app.kestrel.test/{$screen}")
            ->assertOk()
...
it('rejects a salon owner on the console host', function () {
    withSubdomains();
    $tenant = Tenant::factory()->create();
    $owner = App\Models\User::factory()->create(['tenant_id' => $tenant->id, 'is_super_admin' => false]);

    $this->actingAs($owner)->get('http://admin.kestrel.test/')->assertForbidden();
});
...
it('lets a super admin into the console host', function () {
    ...
    $this->actingAs($admin)->get('http://admin.kestrel.test/')->assertOk();
});
...
it('sends a super admin to the console after logging in, not to a diary', function () {
    ...
    $this->post('http://admin.kestrel.test/login', [
        'email' => $admin->email,
        'password' => 'password',
    ])->assertRedirect('http://admin.kestrel.test');
});
...
it('scopes the session cookie to the exact host and never the parent domain', function () {
    ...
    $this->get('http://app.kestrel.test/login');
    expect(config('session.domain'))->toBe('app.kestrel.test')
        ->and(config('session.cookie'))->toBe(App\Support\Surface::App->cookie());

    $this->get('http://admin.kestrel.test/login');
    expect(config('session.domain'))->toBe('admin.kestrel.test')
        ->and(config('session.domain'))->not->toStartWith('.')
        ->and(config('session.cookie'))->toBe(App\Support\Surface::Admin->cookie());
});
...
it('hands impersonation across to the app surface and back again', function () {
    ...
    expect($handoff)->toStartWith('http://app.kestrel.test/impersonate/');

    // Redeeming it issues a normal app session tagged with the impersonator.
    $this->flushSession();
    $this->get($handoff)->assertRedirect('http://app.kestrel.test/diary');

    expect(session('impersonator_id'))->toBe($admin->id)
        ->and(auth()->id())->toBe($owner->id)
        ->and(App\Models\AuditLog::query()->where('action', 'impersonate.start')->count())->toBe(1);

    // Exiting drops the app session and returns to the console.
    $this->post('http://app.kestrel.test/impersonation/stop')
        ->assertRedirect('http://admin.kestrel.test');

    expect(App\Models\AuditLog::query()->where('action', 'impersonate.stop')->count())->toBe(1);
});
...
it('will not let an impersonation link be redeemed twice', function () {
    ...
    $tenant = Tenant::factory()->create();
    App\Models\User::factory()->create(['tenant_id' => $tenant->id, 'role' => App\Enums\UserRole::Owner]);
    $admin = App\Models\User::factory()->create(['tenant_id' => null, 'is_super_admin' => true]);
